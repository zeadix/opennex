...
const out = await page.evaluate(() => {
  const dbg = window.__opennexCursorDebug;
  const dbgInfo = dbg
    ? {
        focused: dbg.focusedSlot?.value,
        slots: Array.from(dbg.cursorBySlot.entries()).map(([k, v]) => ({
          slot: k,
          x: Math.round(v.x),
          y: Math.round(v.y),
          hostLeft: Math.round(v.host.left),
          hostTop: Math.round(v.host.top),
          zoom: v.zoom,
        })),
      }
    : null;
  ...
  let caret = null;
  host?.querySelectorAll(".xterm-rows > div, .xterm-rows span").forEach((row) => {
    if (caret === null && (row.textContent ?? "").includes("$")) {
      ...
      caret = {
        ...
        text: String(row.textContent ?? "").slice(0, 30),

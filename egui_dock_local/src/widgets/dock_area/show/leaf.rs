...
impl<Tab> DockArea<'_, Tab> {
    ...
    fn tab_bar(
        ...
        let actual_width = {
            ...
            // OpenNex patch: when the + button is left-aligned it occupies a
            // fixed slot after the collapse button, so the tabs area shifts
            // right by the same amount and loses that width.
            let plus_left_shift = if self.show_add_buttons
                && matches!(style.buttons.add_tab_align, TabAddAlign::Left)
            {
            ...
            clip_rect.set_width(available_width - plus_left_shift);
            if self.show_leaf_collapse_buttons {
                clip_rect = clip_rect.translate(vec2(Style::TAB_COLLAPSE_BUTTON_SIZE, 0.0));
            }
            clip_rect = clip_rect.translate(vec2(plus_left_shift, 0.0));
    ...
    fn tab_title(
        ...
        let close_response = show_close_button
            .then(|| {
                ...
                // OpenNex patch: the close affordance is only VISIBLE while the
                // pointer is over the tab (or the button itself). Layout keeps
                // reserving the slot so tabs don't resize when hovering.
                let visible = response.hovered() || close_response.hovered();
                if !visible {
                    return None;
                }

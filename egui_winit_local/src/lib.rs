    ...
    pub fn on_window_event(
        ...
        match event {
            ...
            WindowEvent::Focused(focused) => {
                ...
                // LOCAL PATCH (opennex fork): reset the modifier state
                // when the window REGAINS focus. Winit only learns
                // modifiers from `ModifiersChanged`, which the OS will
                // NOT deliver for a release that happened while the
                // window was unfocused (Alt+Tab, WM key grabs, VM/IME
                // quirks) — without this reset a ctrl/alt/super seen as
                // "held" before the switch stays stuck FOREVER:
                // egui-winit then suppresses every Text event (typing
                // dies in terminals) and modifier shortcuts false-fire
                // on plain keys. GTK/Qt apps re-query the display
                // server on focus-in; winit cannot, so default-to-none
                // is the correct approximation — a modifier physically
                // held across the refocus re-asserts itself through its
                // next ModifiersChanged.
                if *focused {
